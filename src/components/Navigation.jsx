...
function Navigation() {
  ...
  return (
    <nav className="">
      <div className="px-4 bg-gray-800 fixed top-0 left-0 w-full z-5000 text-lg">
        <div className="flex justify-between min-h-[100px] items-center mx-auto max-w-[1440px] font-mono tracking-widest">
          <h2 className="font-bold text-2xl">Bikash</h2>
      ...
      {mobileView && (
        <div className="fixed top-0 right-0 h-screen w-full bg-gray-900">
          ...
          <div className="space-y-10 h-full flex flex-col items-center">
            {navItems.map((item, index) => (
              <a
                href={item.to}
                key={index}
